...
export default function Mprofil(navigation, route, props) {
    ...
    const _retrieveData = async () => {
        ...
        fetch(url1)
        return fetch(url2 + '/api/profil?uid='+Id+'&get_profil')
...
export default function Mprofil(navigation, route, props) {
   ...
   /* 
    useEffect(() => {
        var formdata = new FormData()
        formdata.append('uid',"71");
        formdata.append('access_token',"access_token_2ba69fcb414eab5ac4ad74bdf00b4c33587094b5"),
        formdata.append('get_profil',"1"),
        fetch(url1)
        return fetch(url2 + '/api/profil', {
            method: 'POST',
            headers: {
                'Accept':'application/json',
                'Content-Type':'multipart/form-data'
   ...
   /* useEffect(() => {
        fetch(url1)
        return fetch(url2 + '/api/profil?uid=71&get_profil')
            .then((response) => response.json())
            .then((res) => {
                console.log("repooooonse")
                // console.log(res.nom)
